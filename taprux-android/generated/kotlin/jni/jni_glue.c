...
static BoltFFICallbackInitResult init_CruxShell_callbacks(JNIEnv* env);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if (init_CruxShell_callbacks(env) == BOLTFFI_CALLBACK_INIT_FATAL) {
        return JNI_ERR;
    }
    return JNI_VERSION_1_6;
}

JNIEXPORT jbyteArray JNICALL Java_com_ghuba_taprux_core_Native_boltffi_1last_1error_1message(JNIEnv *env, jclass cls) {
    FfiString out = { 0 };
...
JNIEXPORT jlong JNICALL Java_com_ghuba_taprux_core_Native_boltffi_1core_1f_1f_1i_1new(JNIEnv *env, jclass cls, jlong shell) {
    bool _boltffi_input_error = false;
    void* _handle = NULL;
    if (_boltffi_input_error) goto boltffi_input_cleanup;
    _handle = boltffi_core_f_f_i_new(boltffi_create_crux_shell_handle((uint64_t)shell));
boltffi_input_cleanup:
    if (_boltffi_input_error) return 0;
    return (jlong)_handle;
}

JNIEXPORT void JNICALL Java_com_ghuba_taprux_core_Native_boltffi_1core_1f_1f_1i_1free(JNIEnv *env, jclass cls, jlong handle) {
    if (handle != 0) boltffi_core_f_f_i_free((void*)handle);
}
JNIEXPORT jbyteArray JNICALL Java_com_ghuba_taprux_core_Native_boltffi_1core_1f_1f_1i_1update(JNIEnv *env, jclass cls, jlong handle, jbyteArray data) {
    if (handle == 0) return NULL;
    bool _boltffi_input_error = false;

    uintptr_t _data_len = (uintptr_t)(*env)->GetArrayLength(env, data);
    uint8_t* _data_ptr = (uint8_t*)(*env)->GetByteArrayElements(env, data, NULL);
    if (_data_ptr == NULL) { _data_len = 0; _boltffi_input_error = true; }
    FfiBuf_u8 _buf = {0};
    if (_boltffi_input_error) goto boltffi_input_cleanup;
    _buf = boltffi_core_f_f_i_update((void*)handle, (const uint8_t*)_data_ptr, (uintptr_t)_data_len);
boltffi_input_cleanup:
...
JNIEXPORT jbyteArray JNICALL Java_com_ghuba_taprux_core_Native_boltffi_1core_1f_1f_1i_1view(JNIEnv *env, jclass cls, jlong handle) {
    if (handle == 0) return NULL;
    bool _boltffi_input_error = false;
    FfiBuf_u8 _buf = {0};
    if (_boltffi_input_error) goto boltffi_input_cleanup;
    _buf = boltffi_core_f_f_i_view((void*)handle);
boltffi_input_cleanup:
    if (_boltffi_input_error) return NULL;
    return boltffi_buf_to_jbytearray(env, _buf);
}

static jclass g_CruxShell_callbacks_class = NULL;
static jmethodID g_CruxShell_free_method = NULL;
static jmethodID g_CruxShell_clone_method = NULL;
static jmethodID g_CruxShell_process_effects_method = NULL;

JNIEXPORT void JNICALL Java_com_ghuba_taprux_core_Native_boltffiCallbackCruxShellProcessEffects(JNIEnv *env, jclass cls, jlong handle, jobject bytes) {
    if (handle == 0) {
        return;
    }
    bool _boltffi_input_error = false;
    jlong _bytes_size = (*env)->GetDirectBufferCapacity(env, bytes);
    uint8_t* _bytes_ptr = (uint8_t*)(*env)->GetDirectBufferAddress(env, bytes);
    uintptr_t _bytes_len = (_bytes_ptr && _bytes_size > 0) ? (uintptr_t)_bytes_size : 0;
    ...
    const CruxShellVTable* vtable = (const CruxShellVTable*)callback->vtable;
    if (vtable == NULL || vtable->process_effects == NULL) {
        boltffi_throw_runtime(env, "callback method unavailable");
        goto boltffi_input_cleanup;
    }
    FfiStatus _status = {0};
    vtable->process_effects(callback->handle, (const uint8_t*)_bytes_ptr, (uintptr_t)_bytes_len, &_status);
    if (_status.code != 0) {
...
static void CruxShell_vtable_free(uint64_t handle) {
    JNIEnv* env;
    int attached = 0;
    ...
    (*env)->CallStaticVoidMethod(env, g_CruxShell_callbacks_class, g_CruxShell_free_method, (jlong)handle);
    boltffi_consume_pending_exception(env);
    if (attached) (*g_jvm)->DetachCurrentThread(g_jvm);
}

static uint64_t CruxShell_vtable_clone(uint64_t handle) {
    JNIEnv* env;
    int attached = 0;
    jint get_env_result = (*g_jvm)->GetEnv(g_jvm, (void**)&env, JNI_VERSION_1_6);
    if (get_env_result == JNI_EDETACHED) {
    ...
    jlong result = (*env)->CallStaticLongMethod(env, g_CruxShell_callbacks_class, g_CruxShell_clone_method, (jlong)handle);
...
JNIEXPORT jlong JNICALL Java_com_ghuba_taprux_core_Native_boltffiCallbackCruxShellClone(JNIEnv* env, jclass cls, jlong handle) {
    (void)cls;
    return boltffi_jvm_callback_handle_clone(env, boltffi_jvm_callback_handle_ref(handle));
}

JNIEXPORT void JNICALL Java_com_ghuba_taprux_core_Native_boltffiCallbackCruxShellRelease(JNIEnv* env, jclass cls, jlong handle) {
    (void)env; (void)cls;
    boltffi_jvm_callback_handle_release(boltffi_jvm_callback_handle_ref(handle));
}

static void CruxShell_vtable_process_effects(uint64_t handle, const uint8_t* bytes_ptr, uintptr_t bytes_len, FfiStatus* _out_status) {
    JNIEnv* env;
    int attached = 0;
    jint get_env_result = (*g_jvm)->GetEnv(g_jvm, (void**)&env, JNI_VERSION_1_6);
    if (get_env_result == JNI_EDETACHED) {
    ...
    jobject buf_bytes = NULL;
    if (bytes_ptr != NULL) {
        buf_bytes = (*env)->NewDirectByteBuffer(env, (void*)bytes_ptr, (jlong)bytes_len);
        if (buf_bytes == NULL) { boltffi_consume_pending_exception(env); }
    }
    (*env)->CallStaticVoidMethod(env, g_CruxShell_callbacks_class, g_CruxShell_process_effects_method, (jlong)handle, buf_bytes);
    if (boltffi_consume_pending_exception(env)) {
        _out_status->code = 1;
        goto cleanup;
    }
    _out_status->code = 0;
cleanup:
    if (buf_bytes != NULL) (*env)->DeleteLocalRef(env, buf_bytes);
    if (attached) (*g_jvm)->DetachCurrentThread(g_jvm);
}

static CruxShellVTable g_CruxShell_vtable = {
    .free = CruxShell_vtable_free,
    .clone = CruxShell_vtable_clone,
    .process_effects = CruxShell_vtable_process_effects,
};

static BoltFFICallbackInitResult init_CruxShell_callbacks(JNIEnv* env) {
    BoltFFIGlobalClassResult class_result =
        boltffi_lookup_global_class(env, "com/ghuba/taprux/core/CruxShellCallbacks", &g_CruxShell_callbacks_class);
    ...
    if (!boltffi_lookup_static_method(env, g_CruxShell_callbacks_class, "free", "(J)V", &g_CruxShell_free_method)) goto fail;
    if (!boltffi_lookup_static_method(env, g_CruxShell_callbacks_class, "clone", "(J)J", &g_CruxShell_clone_method)) goto fail;
    if (!boltffi_lookup_static_method(env, g_CruxShell_callbacks_class, "process_effects", "(JLjava/nio/ByteBuffer;)V", &g_CruxShell_process_effects_method)) goto fail;
    boltffi_register_crux_shell_vtable(&g_CruxShell_vtable);
    return BOLTFFI_CALLBACK_INIT_OK;
fail:
    (*env)->DeleteGlobalRef(env, g_CruxShell_callbacks_class);
    g_CruxShell_callbacks_class = NULL;
    return BOLTFFI_CALLBACK_INIT_FATAL;
}
JNIEXPORT void JNICALL JNI_OnUnload(JavaVM* vm, void* reserved) {
    ...
    if (g_CruxShell_callbacks_class != NULL) {
        (*env)->DeleteGlobalRef(env, g_CruxShell_callbacks_class);
        g_CruxShell_callbacks_class = NULL;
    }
    g_CruxShell_free_method = NULL;
    g_CruxShell_clone_method = NULL;
    g_CruxShell_process_effects_method = NULL;
}