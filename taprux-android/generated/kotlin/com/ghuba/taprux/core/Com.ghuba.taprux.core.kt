 ...
 */
class CoreFfi private constructor(internal val handle: Long) : AutoCloseable {
    private val closed = AtomicBoolean(false)

    constructor(shell: CruxShell) : this(
        Native.boltffi_core_f_f_i_new(CruxShellBridge.create(shell))
    )

    override fun close() {
        if (!closed.compareAndSet(false, true)) return
        Native.boltffi_core_f_f_i_free(handle)
    }


    fun update(`data`: ByteArray): ByteArray {
        val buf = Native.boltffi_core_f_f_i_update(handle, `data`)
    ...
    fun view(): ByteArray {
        val buf = Native.boltffi_core_f_f_i_view(handle)
            ?: throw FfiException(-1, "Null buffer returned")
        return useWireBytes(buf) { buffer ->
            buffer.remaining().let { n -> ByteArray(n).also { buffer.get(it) } }
        }
...
private object CruxShellHandleMap {
    private val map = java.util.concurrent.ConcurrentHashMap<Long, CruxShell>()
    private val counter = java.util.concurrent.atomic.AtomicLong(1L)

    fun insert(obj: CruxShell): Long {
        val handle = counter.getAndAdd(2L)
        map[handle] = obj
        return handle
    }

    fun get(handle: Long): CruxShell? = map[handle]

    fun remove(handle: Long): CruxShell? = map.remove(handle)
...
object CruxShellCallbacks {

    @JvmStatic
    fun free(handle: Long) {
        CruxShellHandleMap.remove(handle)
    }

    @JvmStatic
    fun clone(handle: Long): Long {
        return CruxShellHandleMap.clone(handle)
    }

    @JvmStatic
    fun process_effects(handle: Long, bytes: ByteBuffer) {
        val impl = CruxShellHandleMap.get(handle) ?: error("CruxShellHandleMap: invalid handle $handle")
        impl.processEffects(run { val reader = WireReader(bytes); reader.readBytes() })
    }
}

private class CruxShellProxy(private val handle: Long) : CruxShell {
    @Suppress("deprecation")
    protected fun finalize() {
        if (handle != 0L) {
            Native.boltffiCallbackCruxShellRelease(handle)
        }
    }
    ...
    override fun processEffects(bytes: ByteArray) {
        val wire_writer_bytes = WireWriterPool.acquire((4 + bytes.size))
            kotlin.run {
                val wire = wire_writer_bytes.writer
                wire.writePrimitiveList(bytes)
            }
        try {
            Native.boltffiCallbackCruxShellProcessEffects(handle, wire_writer_bytes.buffer)
        } finally {
            wire_writer_bytes.close()
        }
    }
}

object CruxShellBridge {
    ...
    fun create(impl: CruxShell): Long {
        register()
        return CruxShellHandleMap.insert(impl)
    }

    fun wrap(handle: Long): CruxShell {
        return CruxShellHandleMap.get(handle) ?: CruxShellProxy(handle)
    }
}
...
private object Native {
    init {
        val androidLibrary = "taprux-core"
        val desktopPreferredLibrary = "taprux_core_jni"
        val desktopFallbackLibrary = "taprux_core"
